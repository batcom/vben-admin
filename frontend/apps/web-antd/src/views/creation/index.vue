...
const columns = [
      ...
      { title: 'result_url', dataIndex: 'result_url', key: 'result_url' },
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getCreationListApi({
...
function handleAdd() {
  formState.value = {};
  modalTitle.value = '创作管理 - 新增';
  modalVisible.value = true;
}

function handleEdit(record: any) {
  formState.value = { ...record };
  modalTitle.value = '创作管理 - 编辑';
  modalVisible.value = true;
}
...
async function handleOk() {
  if (formState.value.id) {
    await updateCreationApi(formState.value.id, formState.value);
  } else {
    await createCreationApi(formState.value);
  }
  ...
  <div class="p-4">
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" @ok="handleOk">
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-item label="result_url" name="result_url">
          <a-input v-model:value="formState.result_url" />
        </a-form-item>
        ...
        <a-form-item label="credits" name="credits">
          <a-input-number v-model:value="formState.credits" style="width:100%" />
        </a-form-item>
