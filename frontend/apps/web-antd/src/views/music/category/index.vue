...
import {
  getMusicCategoryListApi,
  createMusicCategoryApi,
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getMusicCategoryListApi({
...
function handleAdd() {
  formState.value = {};
  modalTitle.value = '音乐分类 - 新增';
  modalVisible.value = true;
}

function handleEdit(record: any) {
  formState.value = { ...record };
  modalTitle.value = '音乐分类 - 编辑';
  modalVisible.value = true;
}

async function handleDelete(id: number) {
  await deleteMusicCategoryApi(id);
...
async function handleOk() {
  if (formState.value.id) {
    await updateMusicCategoryApi(formState.value.id, formState.value);
  } else {
    await createMusicCategoryApi(formState.value);
  }
