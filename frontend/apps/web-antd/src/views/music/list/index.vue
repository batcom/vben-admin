...
async function loadData() {
  loading.value = true;
  try {
    const res = await getMusicListListApi({
...
function handleAdd() {
  formState.value = {};
  modalTitle.value = '用户音乐 - 新增';
  modalVisible.value = true;
}

function handleEdit(record: any) {
  formState.value = { ...record };
  modalTitle.value = '用户音乐 - 编辑';
  modalVisible.value = true;
}

async function handleDelete(id: number) {
  await deleteMusicListApi(id);
...
async function handleOk() {
  if (formState.value.id) {
    await updateMusicListApi(formState.value.id, formState.value);
  } else {
    await createMusicListApi(formState.value);
  }
  ...
  <div class="p-4">
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" @ok="handleOk">
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-item label="category_id" name="category_id">
          <a-input-number v-model:value="formState.category_id" style="width:100%" />
        </a-form-item>
