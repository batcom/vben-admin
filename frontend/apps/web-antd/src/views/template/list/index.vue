...
import {
  getTemplateListListApi,
  createTemplateListApi,
  updateTemplateListApi,
  deleteTemplateListApi,
} from './api';
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getTemplateListListApi({
...
function handleAdd() {
  formState.value = {};
  modalTitle.value = '视频模板 - 新增';
  modalVisible.value = true;
}

function handleEdit(record: any) {
  formState.value = { ...record };
  modalTitle.value = '视频模板 - 编辑';
  modalVisible.value = true;
}

async function handleDelete(id: number) {
  await deleteTemplateListApi(id);
...
async function handleOk() {
  if (formState.value.id) {
    await updateTemplateListApi(formState.value.id, formState.value);
  } else {
    await createTemplateListApi(formState.value);
  }
